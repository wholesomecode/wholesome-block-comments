		...
		<article className={ `${ className } example-block h-entry hentry` } key="Block">
			{ ! imageURL ? (
				<MediaPlaceholder
					...
					labels={ {
						title: 'Upload Image',
						instructions: __( 'Upload an image for the header', 'wholesome-publishing' ),
					} }
			...
			<div className="example-block__body">
				<RichText
					...
					placeholder={ __( 'Title', 'wholesome-publishing' ) }
					tagName="h2"
					value={ title }
				/>
				<RichText
					...
					placeholder={ __( 'Summary', 'wholesome-publishing' ) }
					tagName="div"
					value={ summary }
				/>
			</div>
